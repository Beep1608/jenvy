...
public interface  Page {
    ...
    final ReadOnlyObjectProperty <Consumer<Class<? extends Page>>> slide = new ReadOnlyObjectWrapper<>();
   default  void setNav(Consumer<Class<? extends Page>> nav){
       ((ReadOnlyObjectWrapper<Consumer<Class<? extends Page>>>) this.nav).set( nav);
    }
    default void setSlide(Consumer<Class<? extends Page>> slide){
        ((ReadOnlyObjectWrapper<Consumer<Class<? extends Page>>>) this.slide).set(slide);
    }
   default  void nav(Class<? extends Page> page){
        nav.get().accept(page);
    }

    default void slide(Class<? extends Page> page){
       slide.get().accept(page);
    }
